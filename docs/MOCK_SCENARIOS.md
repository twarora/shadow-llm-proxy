...
| 2 | Same answer, reordered | What is the capital of France? | `The capital of France is Paris.` | `Paris is the capital of France.` | 1.00 | no | **MATCH** | similarity |
...
| 9 | Primary failure | Summarize the quarterly report. | — (HTTP 503) | — | — | — | **PRIMARY_FAILED** | shadow skipped |
| 10 | Shadow failure | Give me today's weather. | `It is sunny and 24 degrees.` | — (timeout) | — | — | — | `shadow_failure` |

## How to read the routing

```
similarity >= 0.90 ?
   ├─ yes → MATCH        (decision_source = semantic_similarity, judge skipped)   ── scenarios 1, 2
   └─ no  → call judge
              ├─ MATCH                      ── scenarios 3, 4
              ├─ MISMATCH → mismatch log    ── scenarios 5, 6, 7
              └─ unparseable → UNKNOWN       ── scenario 8

primary fails  → PRIMARY_FAILED, no shadow                                        ── scenario 9
shadow fails   → shadow_failure, user already served                              ── scenario 10
```
...
|----------------|-----------|
| MATCH via similarity (judge skipped) | 1, 2 |
| MATCH via judge | 3, 4 |
| MISMATCH via judge (logged) | 5, 6, 7 |
| UNKNOWN (judge unparseable) | 8 |
| Primary failure (shadow skipped) | 9 |
| Shadow failure (isolated) | 10 |
...
- Prompts not in the fixtures fall back to default mock behavior (`[mismatch]` / `[slow]` tokens).
